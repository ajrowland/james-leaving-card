...
import { GAME_MESSAGES, GIF_MESSAGES, ENEMY_SPAWN_POSITIONS, GAME_CONFIG } from "../../lib/gameData"
...
export default function GameWorld() {
  ...
  useFrame(() => {
    ...
    if (!levelCompleteRef.current &&
      ...
      const spawn = ENEMY_SPAWN_POSITIONS[Math.floor(Math.random() * ENEMY_SPAWN_POSITIONS.length)].clone()
    ...
    bullets.forEach(bullet => {
      ...
      currentEnemies.forEach(enemy => {
        ...
        if (dist < GAME_CONFIG.HIT_DISTANCE) {
          ...
          const updated = useGameState.getState().enemies.find(e => e.id === enemy.id)
      ...
      boxes.forEach(box => {
        if (!box.hit && bullet.position.distanceTo(box.position) < GAME_CONFIG.HIT_DISTANCE) {
      ...
      gifs.forEach(gif => {
        if (!gif.hit && bullet.position.distanceTo(gif.position) < GAME_CONFIG.HIT_DISTANCE) {
          hitGifBox(gif.id)
    ...
    enemies.forEach(enemy => {
      ...
      if (dist < GAME_CONFIG.ENEMY_ATTACK_RANGE &&
  ...
  return (
    <>
      ...
      {messageBoxes.map(box => (
        <MessageBox key={box.id} position={box.position} hit={box.hit} />
      ))}

      {gifBoxes.map(box => (
        <GifBox key={box.id} id={box.id} position={box.position} imageUrl={box.imageUrl} hit={box.hit} />
      ))}

      {enemies.map(enemy => (
        <Enemy key={enemy.id} position={enemy.position} health={enemy.health} maxHealth={enemy.maxHealth} />
      ))}
